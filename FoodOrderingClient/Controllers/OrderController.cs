    ...
    public class OrderController : ControllerBase
    {
        ...
        public async Task<IActionResult> PlaceOrder([FromBody] CreateOrderDto request)
        {
            try
            {
                ...
                if (uidClaim == null)
                {
                    return Unauthorized(ResultHelper.Error<string>("User claim not found!"));
                }
                ...
                {
                    var food = await _context.Foods.FirstOrDefaultAsync(f => f.Fid == item.Fid);

                    if (food == null)
                    {
                        return BadRequest(ResultHelper.Error<string>($"Food Id {item.Fid} not found!"));
                    }
